...
- **UI** (`src/api/ui.ts`, бейдж `v13 · живой`): вкладки Координатор/Сотрудники/Команда/Админ.
...
1. **Doc-2 глубже**: многораундовые дебаты агентов (сейчас: декомпозиция → вклад каждого →
   **ревью-раунд Revisa с вердиктом** — уже в проде; дальше — итерации «доработай по ревью»).
2. **Doc-3 P1–P3** (см. `agent-os/docs/OFFICE_VISUAL.md`): камера (zoom/pan/follow), комнаты
   (переговорка/серверная/lounge), карточки событий, панели проекта/агента, LOD. P0 — готово (v12).

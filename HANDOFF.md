...
Обновлено: 2026-06-10 (v21; ядро: память диалога, честные ошибки, lifecycle, журнал агента; docs/design/*).
...
- **UI** (`src/api/ui.ts`, бейдж `v21 · каталог команд`): полноэкранный app shell с левым
  сайдбаром (Офис / Задачи / Активность / Сотрудники / Нанять команду / Команда / Админ;
  карточка Координатора, список сотрудников; resize-адаптация, канвас пересоздаётся).
  Офис: спрайты 2.0 (char-grid 16x23, ~54x78px, костюмы, аксессуары роли), камера
  (зум/панорама/dblclick), линии взаимодействия Координатор→агент. Личный чат: серверная
  история, ответы на выбранное сообщение (цитата), вложения, честные ошибки + «Повторить»,
  typewriter. Координатор: полоса фаз + «Обсуждение команды» (переживает перезагрузку).
  Задачи/Активность/Журнал агента с Debug (промпт/шаги/токены). Каталог «Нанять команду»:
  10 шаблонов с мини-аватарами, идемпотентный найм.
  ВАЖНО: внутри template literal нельзя backticks/${} и regex-эскейпы пишутся '\\s'.
...
7. **Бэкапы Postgres** — ✅ ночной `deploy/pg-backup.sh` (сервис `pgbackup`): дампы
   agent_os+teamly с ротацией 14 дней на том `pgbackups`. Офсайт — задать
   `BACKUP_RSYNC_TARGET` в .env (без него бэкап локальный). Проверка восстановления — TODO.
8. **Системные обновления** RU-сервера (`apt upgrade && reboot`, стек поднимется сам).

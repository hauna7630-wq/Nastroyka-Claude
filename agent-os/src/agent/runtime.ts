...
export async function executeRun(runId: string, deps: RuntimeDeps): Promise<void> {
  ...
  if (personalChat) {
    system =
      ...
      'ПОРУЧЕНИЕ <Имя>: <что нужно сделать>. Коллега выполнит это САМ автоматически, и его результат ' +
      'придёт прямо в этот чат — НЕ проси пользователя ничего нажимать. Используй это ТОЛЬКО для ' +
      'реальной передачи, не упоминай эту команду в обычном ответе.\n\n' +
      system;
  }
  if (deps.memory) {

  ...
  },
  {
    ...
    systemPrompt:
      [
        ...
        'Инструменты добычи фактов: веб-поиск и выборка страниц (WebSearch/WebFetch). Если нужны',
        'данные из публичного API — можешь дёрнуть его сам кодом (code_exec: Python/Node) и распарсить.',
  ...
  },
  {
    ...
    systemPrompt:
      [
        ...
        'У тебя ПОСТОЯННАЯ рабочая папка (текущий каталог) — файлы, которые ты создаёшь, сохраняются',
        'между сообщениями. Веди проект пошагово: сначала `ls`/`cat` посмотри, что уже есть, потом',
        'дополняй. Не выдумывай наличие файлов — проверяй реально.',
